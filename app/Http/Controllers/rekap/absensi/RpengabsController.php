...
class RpengabsController extends Controller
{
    ...
    {
        date_default_timezone_set('Asia/Pontianak');
        $rekap_master = Master::where('status', 'Validasi')->first();
        $periode = date('m-Y');
        $today = date('d-m-Y');
        $time = date('H:i');
        ...
        $kar = User::where('status', '<>', 'Hidden')
            ->where('status', '<>', 'Delete')
            ->get();
        return view('asset.sad.rekap.pengabs.pengabs_create_m', compact('kar', 'cek', 'periode', 'master', 'today', 'time', 'abs','rekap_master'));
    }
    ...
    public function absSearch_2(Request $request)
    {
        ...
        return view('asset.sad.rekap.pengabs.pengabs_create_m', compact('abs', 'cek', 'kar', 'today', 'time', 'periode', 'master','rekap_master'));
    }
