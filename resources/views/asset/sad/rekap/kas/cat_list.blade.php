...
@section('judul')
    Catering | Validasi | HWA &bull; SAT
@endsection
    ...
    @if ($cek > 0)
        ...
        <div class="card mb-3">
            <div class="card-header bg-light d-flex flex-between-justify py-2">
                <h3></h3>
                <button type="button" data-bs-toggle="modal" data-bs-target="#setting" class="btn btn-sm btn-warning"><i
                        class="fas fa-cog"></i>
                    Setting</button>&nbsp;
                @if ($cat_m->porsi_harga != null)
                    <form action="{{ route('r.cat.r') }}" method="post">
                        @csrf
        ...
        @include('comp.modal.kas.modal_rekap_cat_setting')
        @include('comp.modal.kas.modal_rekap_cat_edit')
        @include('comp.modal.kas.modal_rekap_cat_hapus')
    @else
